...
将`tab.py`存放在自己python的安装目录下的`Lib`目录下，如 `D:\Program Files (x86)\python3.6.2\Lib` 打开命令行窗口，输入`python`进入python编译环境。
...
如，下面这种使用`from ... import ... as new_name`方式导入`sys`模块中的常量`copyright`，并重命令为`RT`，直接输入`RT`就以打印出版权信息:
